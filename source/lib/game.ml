...
module Game = struct
  ...
  let update_state dt (mouse_x, _) state =
    if not state.running then 
      ...
      if snd ball_after_power_dissipate.pos < snd initial_state.paddle.pos then
        ...
        let direction_x = fst paddle_after_power_dissipate.pos -. fst initial_state.ball.pos +. fst paddle_after_power_dissipate.dim /. 2.0 in
        let direction_y = snd paddle_after_power_dissipate.pos -. snd initial_state.ball.pos in
